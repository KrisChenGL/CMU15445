...
auto Planner::GetFuncCallFromFactory(const std::string &func_name, std::vector<AbstractExpressionRef> args)
  ...
  if(func_name != "lower" && func_name != "upper"){
    throw Exception("func name not lower or upper");
  }

  if (args.size() != 1) {
    throw Exception("args size not 1");
  }

  return std::make_shared<StringExpression>(args[0],func_name == "upper"?StringExpressionType::Upper: StringExpressionType::Lower);
}
