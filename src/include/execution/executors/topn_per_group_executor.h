...
class TopNPerGroupExecutor : public AbstractExecutor {
 ...
 private:
  ...
  /** The result tuples after sorting and limiting */
  std::vector<Tuple> result_tuples_;
  /** Current iterator for result tuples */
  std::vector<Tuple>::iterator current_iter_;
  /** The number of tuples to keep per group */
  size_t n_;
};
}  // namespace bustub
