...
class SkipList {
 ...
 public:
  ...
  explicit SkipList(const Compare &compare = Compare{}) :compare_(compare), header_(std::make_shared<SkipNode>(MaxHeight)){
...
struct SkipList<K, Compare, MaxHeight, Seed>::SkipNode {
  /**
   ...
   */
  explicit SkipNode(size_t height, K key = K{}) : links_(height), key_(key) {}
