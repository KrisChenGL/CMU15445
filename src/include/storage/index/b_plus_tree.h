...
class BPlusTree {
 ...
 private:
  ...
  auto FindLeafPage(const KeyType &key, bool exclusive, Transaction *txn, Context *ctx = nullptr) -> WritePageGuard;
  void StartNewTree(const KeyType &key, const ValueType &value);
  template <typename N>
  auto Split(N *node) -> N *;
  void InsertIntoParent(BPlusTreePage *old_node, const KeyType &key, BPlusTreePage *new_node, Transaction *txn, Context *ctx);
  template <typename N>
  void CoalesceOrRedistribute(N *node, Transaction *txn, Context *ctx);
  template <typename N>
