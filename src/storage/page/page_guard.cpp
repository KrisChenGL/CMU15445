...
ReadPageGuard::ReadPageGuard(page_id_t page_id, std::shared_ptr<FrameHeader> frame,
    ...
    : page_id_(page_id),
      ...
      is_valid_(true){
  // frame_->rwlatch_.lock_shared() 已经在 BufferPoolManager::ReadPage 中调用
...
void ReadPageGuard::Flush() { 
  ...
  DiskRequest request{
    .is_write_ = true,
    .data_ = const_cast<char*>(frame_->GetData()),
    .page_id_ = page_id_,
...
WritePageGuard::WritePageGuard(page_id_t page_id, std::shared_ptr<FrameHeader> frame,
    ...
    : page_id_(page_id),
      ...
      disk_scheduler_(std::move(disk_scheduler)),
      is_valid_(true){
  // frame_->rwlatch_.lock() 已经在 BufferPoolManager::WritePage 中调用
  // UNIMPLEMENTED("TODO(P1): Add implementation.");
}
