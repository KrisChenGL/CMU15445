...
TEST(LRUKReplacerTest, DISABLED_SampleTest) {
  ...
  auto result = lru_replacer.Evict(&evicted_frame);
  ASSERT_TRUE(result.has_value());
  ASSERT_EQ(2, evicted_frame);
  
  result = lru_replacer.Evict(&evicted_frame);
  ASSERT_TRUE(result.has_value());
  ASSERT_EQ(3, evicted_frame);
  
  result = lru_replacer.Evict(&evicted_frame);
  ASSERT_TRUE(result.has_value());
  ASSERT_EQ(4, evicted_frame);
